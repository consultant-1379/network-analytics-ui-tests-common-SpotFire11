...
const waitReady = () =>  cy.get('.sf-svg-loader-12x12').as('status').should('have.attr', 'style').should('contain', 'display: none;')

let countValue = 0;
function waitForReady() {
    cy.get('.sf-svg-loader-12x12').as('status').should('have.attr', 'style').then(e1 => {
        var status = e1
        cy.log(status)
        if (status.includes('display: none;')) {
            cy.log('Page is ready')
            countValue = 0;
        } else {
            cy.wait(5000)
            cy.log('waiting to be ready')
            cy.log('Loop number count : ' + countValue)
            countValue++
            if (countValue > 120) {
                countValue = 0;
                cy.log('Making count value zero : ' + countValue)
                cy.reload()
                ui.waitReady()
                cy.log('**** Page Loaded after 10 Mins ****')
            } else {
                return waitForReady()
            }
        }
...
const pageTitle = () => cy.get(`div[class="sfx_page-title_219"]`)
